...
double eval_pref_sub(int n, int delta, double grav, double sep,
  ...
  for (ny=0;ny<n;ny++){
   for (nx=0;nx<n;nx++){
      //printf("nx is %d and ny is %d when n is %d\n",nx,ny,n);
        ...
        for (dy=MAX(ny-delta,0);dy<MIN(ny+delta+1,n);dy++){
          ...
          float temp_v[3] __attribute__ ((aligned (64))) = {0.0, 0.0, 0.0} ;
           __m128 s_tmp  = _mm_sub_ps(_mm_load_ps(n_d),_mm_load_ps(d_d));
            ...
            // printf("Old values %f and new %f \n",rlen,((double*)&sse_fx)[0] );
                ...
                double *fx, double *fy, double *fz, int no_threads)
{
  ...
  pe = 0.0;
    ...
    pe=eval_pref_sub(n,delta,grav,sep,fcon,x,y,z,fx,fy,fz);
